...
class Regression(object):
    ...
    def __init__(self, df, dependent=None, *independent):
        """
        ...
        self.independent = list(independent)
    ...
    def plot_all(self, interactive=False):
        """
        ...
        if not interactive:
            ...
            data = [{'x': df['date'], 'y': df[dependent], 'name': dependent}]
            for indep in independent:
                data.append({'x':df['date'], 'y':df[indep], 'name':indep})
            iplot(data)

        
    
    def plot2D(self, rotation=False, interactive=False):
        """
        ...
        if not interactive:
            ...
            init_notebook_mode(connected=True)
            trace1 = go.Scatter({'x': df[independent], 'y': df[dependent], 'name': 'scatter', 'mode': 'markers'})
            trace2 = go.Scatter({'x': df[independent], 'y': model, 'name': 'fitted'})
            layout = go.Layout(
    ...
    def residual_plot(self, std_line=2, rotation=True, interactive=False):
        """
        ...
        if not interactive:
            ...
            import plotly.graph_objs as go
            init_notebook_mode(connected=True)
            data = [go.Scatter({'x':df['date'], 'y':self.result.resid, 'name':'residual'})]
            if std_line != 0:
                data.append(go.Scatter({'x':df['date'], 'y':std_line*std*np.ones(num)+mean,
                                        'line':{'color':'red', 'dash':'dash'}, 'showlegend':False}))
                data.append(go.Scatter({'x': df['date'], 'y': -std_line * std * np.ones(num) + mean,
                                        'line': {'color': 'red', 'dash': 'dash'}, 'showlegend':False}))
                layout = go.Layout(title='residual plot ({} STD band)'.format(std_line))
                fig = go.Figure(data=data, layout=layout)
                iplot(fig)

        
    def residual_vs_fit(self, interactive=False):
        ...
        if not interactive:
            ...
            import plotly.graph_objs as go
            init_notebook_mode(connected=True)
            predicts = go.Scatter({'x':y_predict, 'y':residual, 'mode':'markers', 'showlegend':False})
            zeros = go.Scatter({'x':y_predict, 'y':np.zeros(len(residual)),
                                'line':{'color':'red', 'dash':'dash'}, 'showlegend':False})
            layout = go.Layout(
            ...
            fig = go.Figure(data=[predicts, zeros], layout=layout)
            iplot(fig)
    ...
    def test_consistency(self, *args, **kwargs):
        test_data = new_data.cut_data(self.df, *args, **kwargs)
    ...
    def run_all(self, interactive=False):
        """
        ...
        print 
        self.plot_all(interactive=interactive)
        print
        print self.summary()
        if _2D:
            self.plot2D(interactive=interactive)
        print
        print 'Error statistics'
        print self.residual().describe()
        print
        self.train_test_sets()
        self.residual_vs_fit(interactive=interactive)
        self.residual_plot(interactive=interactive)
        residual = self.residual()
